...
legacy_query_client: Optional[KustoClient] = None
if ADX_INGEST_URI and ADX_DATABASE and LEGACY_ADX_TABLE:
    kcsb_legacy = KustoConnectionStringBuilder.with_aad_managed_service_identity_authentication(ADX_QUERY_URI)
    legacy_query_client = KustoClient(kcsb_legacy)
else:
    logging.info("Legacy ADX configuration incomplete; legacy pipeline will be skipped.")


def latest_legacy_room_query(room_id, table) -> str:
    table_ref = format_table_name(table)
    room_filter = ""
    if room_id:
        safe_room = room_id.replace("'", "''")
        room_filter = f"| where RoomID == '{safe_room}'"
    return f"""
{table_ref}
| where isnotempty(RoomID) and isnotempty(Timestamp)
{room_filter}
| summarize arg_max(Timestamp, *) by RoomID
"""


def fetch_latest_legacy_rows(room_id: Optional[str] = None) -> List[Dict[str, Any]]:
    if not (legacy_query_client and ADX_DATABASE and LEGACY_ADX_TABLE):
        return []
    query = latest_legacy_room_query(room_id, LEGACY_ADX_TABLE)
    try:
        result = legacy_query_client.execute(ADX_DATABASE, query)
    except Exception as exc:
        logging.error("Failed legacy ADX query: %s", exc)
        return []
    tables = result.primary_results
    if not tables:
        return []
    return tables[0].to_dict().get("data", [])


def build_patch_from_legacy_record(record: Dict[str, Any]) -> List[Dict[str, Any]]:
    action = record.get("ControlAction", "N/A")
    timestamp = record.get("Timestamp") or datetime.now(timezone.utc).isoformat()
    return [
        {"op": "replace", "path": "/currentTemp", "value": record.get("CurrentTemperature")},
        {"op": "replace", "path": "/targetTemp", "value": record.get("TargetTemperature")},
        {"op": "replace", "path": "/humidity", "value": record.get("Humidity")},
        {"op": "replace", "path": "/occupancy", "value": record.get("Occupancy")},
        {"op": "replace", "path": "/occupantCount", "value": record.get("TotalOccupantCount")},
        {"op": "replace", "path": "/ambientTemp", "value": record.get("AmbientTemperature")},
        {"op": "replace", "path": "/ambientHumidity", "value": record.get("AmbientHumidity")},
        {"op": "replace", "path": "/powerConsumption", "value": record.get("PowerConsumption")},
        {"op": "replace", "path": "/fanSpeed", "value": record.get("FanSpeed")},
        {"op": "replace", "path": "/hvacAction", "value": action},
        {"op": "replace", "path": "/hvacActionColor", "value": map_color(action)},
        {"op": "replace", "path": "/timestamp", "value": timestamp},
    ]
...
@app.event_hub_message_trigger(
    arg_name="legacy_events",
    event_hub_name=EVENT_HUB_NAME,
    connection="EventHubConnectionString",
    consumer_group="adx-legacy-ingestion",

)
def process_legacy_pipeline(legacy_events: List[EventHubEvent]) -> None:
    if not isinstance(legacy_events, list):
        legacy_events = [legacy_events]

    if not (legacy_query_client and LEGACY_ADX_TABLE):
        logging.debug("Legacy pipeline disabled (missing ADX settings).")
        return
    ...
    for event in legacy_events:
        try:
            ...
            logging.error("Legacy pipeline failed to decode Event Hub message: %s", exc)
            continue

        room_id = message.get("RoomID") or message.get("room_id")
        if not room_id:
            logging.warning("Legacy pipeline missing RoomID: %s", message)
            continue

        records = fetch_latest_legacy_rows(room_id)
        if not records:
            continue

        record = records[0]
        patch = build_patch_from_legacy_record(record)
        try:
            adt_client.update_digital_twin(room_id, patch)
            logging.info("Legacy pipeline updated %s with action %s", room_id, record.get("ControlAction", "N/A"))
        except Exception as exc:
            logging.error("Failed to update legacy twin %s: %s", room_id, exc)
